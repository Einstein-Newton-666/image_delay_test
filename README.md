...
延迟口径：发布端可用 `cv::Mat` 到订阅端可用 `cv::Mat` 的端到端延迟。`generate_in_transport_buffer:=true` 时，mode=4/5/6/7 的发布端 `cv::Mat` 直接包装 loaned payload / iceoryx chunk / ROS Image data / auto_aim shm slot，延迟不包含图像填充；`generate_in_transport_buffer:=false` 时，先生成普通 `cv::Mat`，再拷贝到目标传输 buffer，延迟包含 loan/分配、拷贝、publish/take。mode=1/2/3 仍从普通 `cv::Mat` 开始。

CPU 口径：`component_container` 进程级平均 CPU，占 all-core / one-core；包含图像填充、发布、订阅、回调和日志开销，不是仅传输函数局部 CPU。

## 测试结果汇总（2026-06-06 统一重测，mode=6 于 2026-06-07 补测，mode=7 于 2026-06-12 补测）
...
| auto_aim POSIX shm ring | - | 7 | False | - | **0.045 / 0.062ms** | 1.03% / 16.39% | `generate_in_transport_buffer=true, copy_image=false`，直接在单槽 shm 生成图像 |
...
| auto_aim POSIX shm ring | - | 7 | False | - | **1.389 / 1.530ms** | 2.55% / 40.83% | `generate_in_transport_buffer=false, copy_image=false`，源 Mat memcpy 到单槽 shm |
...
- **共享内存 copy 成本可直接对比**：mode=5 direct/copy p50 为 0.110ms / 0.760ms；mode=7 direct/copy p50 为 0.045ms / 1.389ms；mode=4 direct/copy p50 为 2.301ms / 4.141ms。
- **auto_aim 共享内存取图**：mode=7 使用 POSIX `shm_open` + `mmap` + futex 的单槽 ring；发布端直接写 shm slot 后 p50 0.045ms，源 Mat memcpy 到 shm 时 p50 1.389ms，订阅端 clone 旧结果 p50 2.081ms。
- **mode=7 的实现已拆成独立包**：共享内存 ring 头文件位于 `src/autoaim_shm_image_transport/`，`image_test` 只作为 benchmark 使用方。
- **最快 IPC/传输路径**：mode=7 direct、mode=3、mode=5 direct、mode=6 direct、mode=5 copy、mode=7 copy、mode=2、mode=6 copy 的 p50 分别为 0.045ms、0.076ms、0.110ms、0.121ms、0.760ms、1.389ms、1.588ms、1.707ms。
...
ros2 launch image_test image_test.launch.py mode:=7 image_pub_frequency:=200 copy_image:=false generate_in_transport_buffer:=true
ros2 launch image_test image_test.launch.py mode:=7 image_pub_frequency:=200 copy_image:=false generate_in_transport_buffer:=false
ros2 launch image_test image_test.launch.py mode:=7 image_pub_frequency:=200 copy_image:=true generate_in_transport_buffer:=false
```
...
| 零拷贝 wrapper（直接写入目标 buffer） | 4/5/7 | 消除普通 Mat 到共享内存/loaned payload 的中间拷贝 |
| 目标 `cv::Mat` 可用后打时间戳 | 4/5/7 | 统计目标 buffer 可用到订阅端可用的路径，排除图像填充 |
| 源 `cv::Mat` 可用后打时间戳 | 1/2/3，4/5/7 copy | 统计源 Mat 到订阅端可用路径，包含消息创建、拷贝和 publish/take |
