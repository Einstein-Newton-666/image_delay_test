...
- **autoaim_shm_image_transport**：独立头文件包，移植自 auto_aim 的 `shm_open` + `mmap` + futex 单槽共享内存取图（延迟 1.4-2.7ms）
...
# 构建单个包（如 image_test）
colcon build --symlink-install --packages-select image_test
...
- mode=7 是单槽共享内存 ring；订阅端若长期持有零拷贝 Mat，会阻塞发布端覆盖该槽，业务处理较慢时应使用 `copy_image:=true`
