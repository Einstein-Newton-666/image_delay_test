...
ROS 2 工作空间，用于对比测试五种图像传输方式的延迟和 CPU 占用率：
...
# 启动参数说明：
#   mode: 1=ros2, 2=shm_video_transmission, 3=UltraMultiThread, 4=loaned_msg+shm_msg, 5=iceoryx直接通信（默认4）
...
- **image_test**：主测试包。包含 `image_pub` 和 `image_sub` 两个 ROS 2 Component，通过 `mode` 参数切换四种传输方式。
